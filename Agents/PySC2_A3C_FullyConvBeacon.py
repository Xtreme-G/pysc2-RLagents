...
def process_observation(observation):
    ...
    # spatial features
    minimap_channels = len(USED_MINIMAP_FEATURES)
    screen_channels = len(USED_SCREEN_FEATURES)
    minimap_stack = np.resize([features['feature_minimap'][f.name] for f in USED_MINIMAP_FEATURES],
                              (1, FLAGS.minimap_resolution, FLAGS.minimap_resolution, minimap_channels))
    screen_stack = np.resize([features['feature_minimap'][f.name] for f in USED_SCREEN_FEATURES],
...
USED_SCREEN_FEATURES = [SCREEN_FEATURES.selected]  #SCREEN_FEATURES.player_relative] #, SCREEN_FEATURES.selected]
USED_MINIMAP_FEATURES = [MINIMAP_FEATURES.selected]  #MINIMAP_FEATURES.player_relative] #, MINIMAP_FEATURES.selected]
...
class ACNetwork:
    def __init__(self, scope, trainer, action_spec):
        with tf.variable_scope(scope):
            ...
            minimap_channels = len(USED_SCREEN_FEATURES)
            screen_channels = len(USED_MINIMAP_FEATURES)
...
class Worker:
    ...
    def train(self, rollout, sess, gamma, bootstrap_value):
        ...
        feed_dict = {
            ...
            self.local_AC.inputs_nonspatial: np.reshape([obs_nonspatial[i][0] for i in range(len(obs_nonspatial))],
                                                        (len(obs_nonspatial), nonspatial_size)),
